...
class SatellitesFilterStrategy(ABC):
    ...
    @abstractmethod
    def is_in_view(
        self, satellite_position: Position, antenna_position: Position
    ) -> bool:
        pass
...
class SatellitesInterferenceFilter:
    ...
    def run(self) -> list[list[PositionTime]]:
        ...
        for antenna_position in self._antenna_positions_by_time:
            for satellite_position in self._sort_satellite_positions_by_time(
                satellite_positions=antenna_position.satellite_positions
            ):
                if satellite_position.time >= self._cutoff_time:
                    break

                in_view = self._filter_strategy.is_in_view(
                    satellite_position.position,
                    antenna_position.antenna_direction.position,
                )
    ...
    @staticmethod
    def _sort_satellite_positions_by_time(
        satellite_positions: list[PositionTime],
    ) -> list[PositionTime]:
...
class SatellitesWithinMainBeamFilter(SatellitesFilterStrategy):
    def is_in_view(
        ...
        return self._is_within_beam_width_altitude(
            satellite_position.altitude, antenna_position.altitude
        ) and self._is_within_beam_with_azimuth(
            satellite_position.azimuth, antenna_position.azimuth
        )

    def _is_within_beam_width_altitude(
        self, satellite_altitude: float, antenna_altitude: float
    ) -> bool:
    ...
    def _is_within_beam_with_azimuth(
        self, satellite_azimuth: float, antenna_azimuth: float
    ) -> bool:
        positions_to_compare_original = [satellite_azimuth, antenna_azimuth]
        positions_to_compare_next_modulus = (
            numpy.array(positions_to_compare_original) + DEGREES_IN_A_CIRCLE
        ).tolist()
        positions_to_compare = itertools.combinations(
            positions_to_compare_original + positions_to_compare_next_modulus, 2
        )
        return any(
            [
                isclose(*positions, abs_tol=self._facility.half_beamwidth)
                for positions in positions_to_compare
            ]
        )
