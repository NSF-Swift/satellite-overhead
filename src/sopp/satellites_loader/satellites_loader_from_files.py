...
class SatellitesLoaderFromFiles(SatellitesLoader):
    ...
    def load_satellites(self) -> list[Satellite]:
        return (
            self._populate_satellites_with_frequency_data()
            if self._satellites_freq_data
            else self._satellites_from_tle
        )
    ...
    def _populate_satellites_with_frequency_data(self):
        return [
            replace(
                satellite,
                frequency=self._satellites_freq_data.get(
    ...
    def _satellites_freq_data(self):
        return (
            self.frequency_file
            and GetFrequencyDataFromCsv(filepath=self.frequency_file).get()
        )
