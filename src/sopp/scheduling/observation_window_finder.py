...
class WindowFinder:
    ...
    def search(self) -> list[SuggestedReservation]:
        ...
        search_start_time = self._ideal_reservation.time.begin - (
            self._search_window / 2
        )
        search_end_time = self._ideal_reservation.time.begin + (self._search_window / 2)
        search_window_res = Reservation(
            facility=self._ideal_reservation.facility,
            time=TimeWindow(begin=search_start_time, end=search_end_time),
            frequency=self._ideal_reservation.frequency,
        )
        overhead_satellites = self._satellites_overhead(search_window_res)
        potential_time_windows = [
            TimeWindow(
                begin=start_time, end=start_time + self._ideal_reservation.time.duration
            )
        ...
        for reservation_window in potential_time_windows:
            ...
            for interference_window in overhead_satellites:
                if (
                    (
                        reservation_window.begin
                        <= interference_window.overhead_time.begin
                        <= reservation_window.end
                    )
                    or (
                        reservation_window.begin
                        <= interference_window.overhead_time.end
                        <= reservation_window.end
                    )
                    or (
                        interference_window.overhead_time.begin
                    ...
                    and (
                        interference_window.overhead_time.end >= reservation_window.end
                    )
                ):
    ...
    def find(self) -> list[SuggestedReservation]:
        ...
        potential_reservations = [
            ...
            for time in potential_time_windows
        ]
        overhead_satellites = [
            self._satellites_overhead(reservation=reservation)
            for reservation in potential_reservations
        ]
        sort_indices = sorted(
            range(len(potential_reservations)),
            key=lambda index: len(overhead_satellites[index]),
        )
        return [
            SuggestedReservation(
                ideal_reservation=self._ideal_reservation,
    ...
    def _satellites_overhead(self, reservation: Reservation) -> list[OverheadWindow]:
        return EventFinderRhodesmill(
            ...
            antenna_direction_path=[
                PositionTime(altitude=0, azimuth=0, time=reservation.time.begin)
            ],
    ...
    def _potential_start_times(self) -> list[datetime]:
        number_of_slots = int(
            self._search_window.total_seconds()
            / self._start_time_increments.total_seconds()
        )
    ...
    def _get_time_slot(self, index: int) -> datetime:
        ...
        return (
            self._ideal_reservation.time.begin
            + backwards_forwards * self._start_time_increments * multiplier
        )
