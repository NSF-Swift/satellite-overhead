...
class GraphGenerator:
    ...
    (generated by the EventFinderSkyfield.interference_windows) and the satellites
    visible above the horizon (generated by EventFinderSkyfield.satellites_above_horizon).
    """
    ...
    def generate_graph(self):
        ...
        plt.bar(
            size_x_axis - 0.2,
            self._number_of_satellites_above_horizon,
            0.4,
            label="Satellites above the horizon",
        )
        plt.bar(
            size_x_axis + 0.2,
            self._number_of_satellites_in_main_beam,
            0.4,
            label="Satellites crossing main observation beam",
        )
        plt.xlabel("Hour (UTC)")
        plt.ylabel("Number of satellites")
        title = "Overhead Satellites at HCRO starting at " + str(
            self._search_window_start
        )
        plt.title(title)
        plt.legend()
    ...
    def _number_of_satellites_above_horizon(self) -> list[int]:
        return self._get_satellites_in_interval_window(
            overhead_windows=self._satellite_above_horizon
        )

    @cached_property
    def _number_of_satellites_in_main_beam(self) -> list[int]:
        return self._get_satellites_in_interval_window(
            overhead_windows=self._interference_windows
        )

    def _get_satellites_in_interval_window(
        self, overhead_windows: list[OverheadWindow]
    ) -> list[int]:
        return [
            len(
                [
    ...
    def _interval_windows(self) -> list[TimeWindow]:
        return [
            TimeWindow(begin=interval, end=interval + timedelta(minutes=59))
